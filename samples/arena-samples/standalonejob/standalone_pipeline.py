...
def sample_pipeline(learning_rate='0.01',
  ...
  train = arena.standalone_job_op(
    ...
    gpus=gpus,
    data=data,
    command="echo %s;python code/tensorflow-sample-code/tfjob/docker/mnist/main.py --max_steps 500 --data_dir /training/dataset/mnist --log_dir /training/output/mnist  --learning_rate %s --dropout %s" % (prepare_data.outputs['train'], learning_rate, dropout),
  ...
  export_model = arena.standalone_job_op(
    ...
    env=["GIT_SYNC_REV=%s" % (commit)],
    data=data,
    command="echo %s;python code/tensorflow-sample-code/tfjob/docker/mnist/export_model.py --model_version=%s --checkpoint_path=/training/output/mnist /training/output/models" % (train.outputs['train'], model_version))
